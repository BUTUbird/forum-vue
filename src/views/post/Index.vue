    ...
    <el-card shadow="never">
      <div slot="header" class="clearfix">
        <el-tabs v-model="activeName" @tab-click="handleClick">
                  ...
                  <div class="level-left">
                    <div class="level-left">
                      <router-link class="level-item" :to="{ path: `/member/${item.username}/home` }">
                        <el-tag size="small">{{ item.alias }}</el-tag>
                      </router-link>
                      ...
                      <span class="is-hidden-mobile">浏览:
                        <el-tag size="small">
                        {{ item.view }}
                        </el-tag>
                      </span>
          ...
          <el-tab-pane label="关注用户发帖" name="like">
            <article v-for="(item, index) in articleList" :key="index" class="media">
              <div class="media-left">
                <figure class="image is-48x48">
                  <img :src="item.avatar" style="border-radius: 5px;">
...
<script>
import {getLike, getList} from '@/api/post'
...
export default {
  ...
  methods: {
    init(tab) {
      if (tab === "like"){
        getLike(this.page.current, this.page.size,this.user.id).then(res => {
          const { data } = res
